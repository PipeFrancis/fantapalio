...
//         const createGameCard_td3 = (game, score, stats_td3, memestats_td3 = []) => {
//             const card = document.createElement('div');
//             card.classList.add('game-card', `cardclass${selectedPlayer.team}`);
...
document.addEventListener("DOMContentLoaded", function() {
    ...
    // --- Helper function for standard game card ---
    const createGameCard = (game, score, game_stats, memestats, selectedPlayer) => {
        const card = document.createElement('div');
        card.classList.add('game-card', `cardclass${selectedPlayer.team}`);
        let statsHtml = `
            <h3>${game}</h3>
    ...
    // --- Helper function for TD3 game card ---
    const createGameCard_td3 = (game, score, stats_td3, memestats_td3 = [], selectedPlayer) => {
        const card = document.createElement('div');
        card.classList.add('game-card', `cardclass${selectedPlayer.team}`);
    ...
    function updateDisplay() {
        const playerIndex = parseInt(select.value, 10);
        ...
        if(what_day_is_it >= 1) 
            cardsContainer.appendChild(createGameCard('G1', selectedPlayer.g1, selectedPlayer.stats_g1, selectedPlayer.memestats_g1, selectedPlayer));
        if(what_day_is_it >= 2) 
            cardsContainer.appendChild(createGameCard('G2', selectedPlayer.g2, selectedPlayer.stats_g2, selectedPlayer.memestats_g2, selectedPlayer));
        if(what_day_is_it >= 3) 
            cardsContainer.appendChild(createGameCard('G3', selectedPlayer.g3, selectedPlayer.stats_g3, selectedPlayer.memestats_g3, selectedPlayer));
        if(what_day_is_it >= 4) 
            cardsContainer.appendChild(createGameCard('Semifinale', selectedPlayer.semi, selectedPlayer.stats_semi, selectedPlayer.memestats_semi, selectedPlayer));
        if(what_day_is_it >= 5) 
            cardsContainer.appendChild(createGameCard_td3('Tiro da 3', selectedPlayer.td3, selectedPlayer.stats_td3, selectedPlayer.memestats_td3, selectedPlayer));
        if(what_day_is_it >= 6) 
            cardsContainer.appendChild(createGameCard('Finale', selectedPlayer.final, selectedPlayer.stats_final, selectedPlayer.memestats_final, selectedPlayer));
    ...
    // --- INITIAL RENDER ---
    // Select the first player in the list on initial load and render their cards immediately
